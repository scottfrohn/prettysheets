#' gs4mattr: Cell Formatting and Conditional Formatting for googlesheets4
#'
#' `gs4mattr` extends `googlesheets4` with a formatting layer for Google
...
#'   \item [gs4mattr_batch()] — batch several formatting calls into one API request
#' }
#'
...
#' styling and freezes row 1 automatically — `gs4mattr` doesn't duplicate
...
#' a Sheets UI-only feature), so `gs4mattr` doesn't offer one.
#'
#' @keywords internal
"_PACKAGE"
